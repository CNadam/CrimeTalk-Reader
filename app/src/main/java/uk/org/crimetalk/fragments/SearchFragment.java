...
public class SearchFragment extends ListFragment implements LoaderManager.LoaderCallbacks<List<ArticleListItem>>,
     ...
     */
    public static SearchFragment newInstance(ArrayList<Parcelable> parcelableArrayList, String emptyText) {

        final SearchFragment searchFragment = new SearchFragment();

        final Bundle bundle = new Bundle();
        bundle.putParcelableArrayList(SearchActivity.ARG_ARTICLE_LIST_HELPER_LIST, parcelableArrayList);
        bundle.putString(SearchActivity.ARG_EMPTY_TEXT, emptyText);
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        // Get rid of loading icon and show text instead
        this.setListShown(true);
        this.setEmptyText(getArguments().getString(SearchActivity.ARG_EMPTY_TEXT));
    ...
    /**
     * Search through a List of {@link uk.org.crimetalk.fragments.helpers.ArticleListHelper}.
     *
     * @param query The {@link java.lang.String} query to search for
     * @param restartLoad true if the Loader be restarted
     */
    public void search(String query, boolean restartLoad) {
        ...
        // If Loader should restart (i.e. new query) restart the Loader
        if (restartLoad) {

            getLoaderManager().restartLoader(0, bundle, this);

        // Loader should not restart, we might be recovering from orientation change so reattach Loader callbacks
        } else {
